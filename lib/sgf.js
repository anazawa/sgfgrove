(function (window) {
  ...
  SGF.parse = (function () {
    ...
    var collection = function () {
      ...
      (function parse (subtrees, ff, props) {
        foreach(subtrees, function (tree) {
          ...
          parse( tree[tree.length-1], ff, props );
        });
      }(trees));
    ...
    (function stringify (subtrees, ff, props) {
      ...
      foreach(subtrees, function (tree) {
        ...
        if ( subtrees === trees ) { // direct descendant of Collection
          ...
          ff = root.hasOwnProperty('FF') ? FF.types.Number.stringify(root.FF) : ['1'];
          ff = parseInt( ff[0], 10 );

          gm = root.hasOwnProperty('GM') ? FF.types.Number.stringify(root.GM) : ['1'];
          gm = parseInt( gm[0], 10 );
        ...
        stringify( tree[tree.length-1], ff, props ); // add sub GameTrees
