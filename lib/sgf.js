...
(function (window) {
  ...
  SGF.FF[4][1] = (function () {
    ...
    /**
     * Inherits all types from {@link SGF.FF[4].TYPES|FF[4].TYPES}.
     * @namespace TYPES
     * @memberof SGF.FF[4][1]
     */
    ...
    /**
     * Represents an intersection of a go board.
     * @constant Point
     * @memberof SGF.FF[4][1].TYPES
     * @example
     * Point.parse(["aa"]); // => "aa"
     * Point.parse(["aa", "bb"]); // TypeError
     * @example
     * Point.stringify("aa"); // => ["aa"]
     * Point.stringify([0, 0]); // TypeError
     */
    TYPES.Point = FF.TYPES.scalar({
      name: 'Point',
      like: /^[a-z]{2}$/i
  ...
  /**
   * Given a SGF string, returns an array of game trees.
   * The supported file formats are {@link SGF.FF[1]|FF[1]}, FF[3] and FF[4].
   * You can also pass a callback function that is used to filter property
   * values. The callback is called with two parameters; the name of the
  ...
  SGF.parse = (function () {
    ...
    var collection = function (string) {
      var trees = [];
