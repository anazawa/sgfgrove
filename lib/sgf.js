...
(function (window) {
   ...
   * SGF string nor decodes the text properties encoded in the specified
   * charset, such as C (comment). You have to decode them by yourself.
   *
  ...
  SGF.parse = (function () {
    ...
    var gameTree = function () {
      ...
      while ( test(/^;\s*/g) ) { // start of Node
        ...
        while ( ident = exec(/^([a-zA-Z\d]+)\s*/g) ) { // PropIdent
          ident = ident[1];
          values = [];
      ...
      while ( test(/^\(\s*/g) ) { // start of sub GameTree
    ...
    var finalize = function (trees, collection, ff, PROPS) {
      var i, tree, root, gm;
      var j, node, idents;
      var k, ident, id;
      ...
      for ( i = 0; i < trees.length; i++ ) {
        ...
        for ( j = 0; j < tree.length-1; j++ ) {
          ...
          for ( k = 0; k < idents.length; k++ ) {
            ...
            node[ident] = (PROPS[ident] || PROPS.unknown).parse(node[ident]);
          }
        }

        finalize( tree[tree.length-1], collection, ff, PROPS );
      }
    };
    ...
    return function (text, reviver) {
      ...
      while ( test(/^\s*\(\s*/g) ) { // start of GameTree
        trees.push( gameTree() );
      }
  ...
  SGF.stringify = (function () {
    ...
    var stringify = function (trees, collection, ff, PROPS) {
      var text = '';
      var i, tree, root, gm;
      ...
      for ( i = 0; i < trees.length; i++ ) {
        ...
        if ( trees === collection ) { // direct descendant of Collection
          root = (tree[0] && typeof tree[0] === 'object') ? tree[0] : {};

          ff = root.hasOwnProperty('FF') ? root.FF : 1;
          ff = Num.parse( Num.stringify(toSGF(ff)) );

          gm = root.hasOwnProperty('GM') ? root.GM : 1;
          gm = Num.parse( Num.stringify(toSGF(gm)) );
        ...
        text += stringify( tree[tree.length-1], collection, ff, PROPS );
